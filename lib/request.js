...
export async function getCountry(req, ip) {
  ...
  if (!lookup) {
    ...
    lookup = await maxmind.open(path.resolve('public/GeoLite2-City.mmdb'));
  }

  const result = lookup.get(ip);
  return {
    country: country ? country : result?.country?.iso_code,
...
export async function getClientInfo(req, { screen }) {
  ...
  const { country, city } = await getCountry(req, ip);
  const browser = browserName(userAgent);
  const os = detectOS(userAgent);
  const device = getDevice(screen, browser, os);

  return { userAgent, browser, os, ip, country, city, device };
}
